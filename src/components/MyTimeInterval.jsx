   ...
   return flight.icao24 &&
    ...
    const userRows = filteredUsers.slice(startIndex, endIndex).map((flight) => (
        <TableRow key={flight.icao24}>
            ...
            <TableCell>{new Date(flight.firstSeen * 1000).toLocaleDateString('en-US', {timeZone: 'GMT', day: '2-digit', month: '2-digit', year: 'numeric'})}, {new Date(flight.firstSeen * 1000).toLocaleTimeString('en-US', {timeZone: 'GMT'})}</TableCell>

            <TableCell>{new Date(flight.lastSeen * 1000).toLocaleString('en-US', {timeZone: 'GMT', hour12: true})}</TableCell>
          </TableRow>
      ));
